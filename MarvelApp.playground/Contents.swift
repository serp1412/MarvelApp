...
let hash = (date + "d5aaf682f856e3668f0ac674484c3373fb790f9e" + "a63d192034338073fa3c6f3d8ac2342f").utf8.md5
let urlString = "https://gateway.marvel.com:443/v1/public/characters?orderBy=-modified&ts=\(date)&hash=\(hash)&apikey=a63d192034338073fa3c6f3d8ac2342f"

URLSession.shared.dataTask(with: URL(string: urlString)!) { (data, response, _) in
    print(response)
    let res = try! JSONDecoder().decode(APIResponse<APICollection<MarvelCharacter>>.self, from: data!)
    print(res)
}.resume()
